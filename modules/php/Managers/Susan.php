...
class Susan
{
  ...
  public static function hasEmptyDepot()
  {
    ...
    for ($i = 0; $i < 6; $i++) {
      if (
        !Tiles::getTopOf('top-interior-' . $depots[$i]['interior'])->first() &&
        !Tiles::getTopOf('top-exterior-' . $depots[$i]['exterior'])->first()
      ) {
        return true;
      }
    }
  ...
  public static function getTilesNumberInMinDeck()
  {
    ...
    for ($i = 0; $i < 6; $i++) {
      $tileInDepot =
        Tiles::countInLocation('interior-' . $i) +
        Tiles::countInLocation('top-interior-' . $i) +
        Tiles::countInLocation('exterior-' . $i) +
        Tiles::countInLocation('top-exterior-' . $i);
